...
namespace MathExtensions
{
    ...
    public struct RightTriangleAbstract
    {
        ...
        private void _FromSideAAngleA(decimal lengthA, decimal angleA)
        {
            ...
            // tan(A) = LB / LA
            // LB = tan(A) * LA
            _lengthB = MathExt.Tan(MathExt.ToRad(angleA)) * lengthA;
            _angleB = 90m - angleA;

            // cos(A) = LA / H
            // H = LA / cos(A)
            _hypotenuse = lengthA / MathExt.Cos(MathExt.ToRad(angleA));
        ...
        private void _FromSideBAngleB(decimal lengthB, decimal angleB)
        {
            ...
            // tan(B) = LA / LB
            // LA = tan(B) * LB
            _lengthA = MathExt.Tan(MathExt.ToRad(angleB)) * lengthB;
            _angleA = 90m - angleB;

            // cos(B) = LB / H
            // H = LB / cos(B)
            _hypotenuse = lengthB / MathExt.Cos(angleB);
